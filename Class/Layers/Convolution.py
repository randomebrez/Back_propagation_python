...
class ConvolutionFFTLayer(LayerBase.__LayerBase):
    ...
    def update_weights(self, previous_layer_activation, learning_rate):
        ...
        # Shape filters and BP activated values for FFT convolution
        shaped_input, shaped_bp_values = self.dw_compute_shape(previous_layer_activation, back_activation_values, kernel_size, stride, zero_padding, filter_number, input_shape)

        # FFT conv
        conv_res = self.convolve_filters(shaped_bp_values, shaped_input)

        # Shape conv result
        shaped_result = self.dw_conv_res_shape(conv_res, back_activation_values, kernel_size, stride, zero_padding, filter_number, input_shape)
    ...
    def dw_compute_shape(self, previous_layer_activation, back_prop_activated_values, kernel_size, stride, padding, filter_number, input_shape):
    ...
    def dw_conv_res_shape(self, convolution_result, back_prop_activated_values, kernel_size, stride, padding, filter_number, input_shape):
        input_depth, input_x, input_y = input_shape[0], input_shape[1], input_shape[2]
        kernel_size_x, kernel_size_y = kernel_size[1], kernel_size[2]
        stride_x, stride_y = stride[1], stride[2]
        padding_depth, padding_x, padding_y = padding[0], padding[1], padding[2]
    ...
    def de_compute_shape(self, filters, back_prop_activated_values, filter_number, kernel_size, stride, input_shape):
        ...
        for i in range(batch_size):
            ...
            shaped_bp_values[::input_depth, index_start:index_start + x_shift - kernel_size[1]:stride_x, ::stride_y] = np.flip(back_prop_activated_values[i], 0)
    ...
    def de_conv_res_shape(self, convolution_result, batch_size, kernel_size, filter_number, padding, input_shape):
        ...
        clean_mean_values = mean_values[:, :-1]
        # last one don't have a marker column to remove
        clean_mean_values += single_split[-1]
